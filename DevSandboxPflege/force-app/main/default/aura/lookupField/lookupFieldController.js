({
    ...
    onFocus : function(component,event,helper){
        ...
        if (component.get("v.showRecent") && searchText.trim() == '') {
            ...
            action.setCallback(this, function(response) {
                var results = response.getReturnValue();
                if (results != null) {
                    component.set("v.statusMessage", results.length > 0 ? null : 'Keine aktuellen Daten.' );
                    component.set("v.searchResult", 
                                  helper.processResults(results, returnFields));
                } else {
                    component.set("v.statusMessage", "Fehler in der Suche!" );
                }
    ...
    onKeyUp : function(component, event, helper) {
        ...
        action.setCallback(this, function(response) {
            var results = response.getReturnValue();
            if (results != null) {
                component.set("v.statusMessage", results.length > 0 ? null : 'Keine Daten gefunden.' );
                component.set("v.searchResult", 
