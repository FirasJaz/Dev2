({
    ...
    createNewOrder : function(component, liefertag, rythmus, glove1, quantity1, glove2, quantity2){
        ...
        var pumpaufsatz = component.get('v.pumpaufsatz');
    ...
    getSpruehkopfProduct: function(component){
       ...
       // get all curaboxes to display
    getPumpaufsatzProduct: function(component){
        var action = component.get('c.getPumpaufsatz');
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state == "SUCCESS") {
                var res = response.getReturnValue();
                if(res != null){
                    component.set('v.pumpaufsatz', res);
                }  
            }
            else{
                console.log(state);
